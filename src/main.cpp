...
#if defined(NDEBUG)
# error "statky cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "statky Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
	...
	if (nHeight <= 10)
	...
	else if (nHeight < 18)  //aprox 1 month, reward halves
			nSubsidy = 50 * COIN;		
		...
		nSubsidy = 10 * COIN;  //until Block 25
...
static const int64_t nTargetTimespan = 10 * 30;  // 16 mins
...
bool CBlock::AcceptBlock()
{
    ...
    else if (!IsProtocolV2(nHeight) && nVersion > 8)
...
#ifdef ENABLE_WALLET
// statky: attempt to generate suitable proof-of-stake
