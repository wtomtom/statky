...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest, nCoinAge, 0, 0);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "statky is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "statky is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "statky is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "statky is downloading blocks...");
