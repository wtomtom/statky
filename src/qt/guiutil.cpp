...
{
    // Convert statky:// to statky:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("statky://"))
    {
        uri.replace(0, 12, "statky:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "statky.lnk";
}
...
{
    return GetAutostartDir() / "statky.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("statky-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  statky-qt [" + tr("command-line options") + "]                     " + "\n";
